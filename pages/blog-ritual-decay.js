...
updateRitual = function updateRitualWithDecay(dt) {
  ...
  ritualCard.classList.toggle('decaying', !ritualComplete && !inside && progress > 0);

  if (ritualComplete) {
    ritualState.textContent = '마법진이 완전히 결속되어 유지됩니다. 중앙을 벗어나도 더 이상 역순으로 해체되지 않습니다.';
  } else if (!inside && progress <= 0) {
    ritualState.textContent = '세 번째 원 안에 머무르면 어두운 구역에서 별자리가 하나씩 깨어납니다.';
  } else if (!inside) {
    ritualState.textContent = `공명이 역순으로 풀리고 있습니다. ${Math.round(progress * 100)}% · 완성 전에는 다시 원 안으로 들어가야 유지됩니다.`;
    ...
    ritualState.textContent = '별자리망, 룬, 고리가 하나의 마법진으로 결속되고 있습니다.';
  }
};
