...
class Satellite {
    constructor(planetWrapper) {
        ...
        this.items = this.satellites.map((satellite, index) => {
            ...
            return {
                ...
                speed: Number(planetWrapper.dataset[`${prefix}Speed`] || (second ? .34 : .42)) * 1.4 * (second ? 4 : 1),
    ...
    createWorldTrail(item) {
        for (let i = 0; i < item.trailLength; i++) {
            ...
            Object.assign(element.style, { position:"absolute", width:"2.5px", height:"2.5px", borderRadius:"50%", pointerEvents:"none", background: item.satellite.classList.contains("link-satellite-square") ? "rgba(255,210,90,.95)" : "rgba(79,216,255,.95)", boxShadow: item.satellite.classList.contains("link-satellite-square") ? "0 0 8px rgba(255,190,60,.9)" : "0 0 8px rgba(79,216,255,.85)", opacity:"0", zIndex:"2" });
    ...
    update(dt) {
        ...
        for (const item of this.items) {
            ...
            item.trailTimer += dt;
            if (item.trailTimer >= .5) { item.trailTimer = 0; this.recordWorldPosition(item, planet); }
    ...
    recordWorldPosition(item, planet) {
        ...
        for (let i=item.trail.length-1;i>0;i--) { item.trail[i].x=item.trail[i-1].x; item.trail[i].y=item.trail[i-1].y; item.trail[i].z=item.trail[i-1].z; item.trail[i].initialized=item.trail[i-1].initialized; }
        item.trail[0].x=planet.x+satellite.x; item.trail[0].y=planet.y; item.trail[0].z=satellite.z; item.trail[0].initialized=true;
    }

    renderWorldTrail(item) {
        item.trail.forEach((point,index)=>{ if(!point.initialized){point.element.style.opacity="0";return;} const fade=1-index/item.trail.length,depth=clamp((point.z+item.depth)/(item.depth*2),0,1); point.element.style.left=`${point.x}px`;point.element.style.top=`${point.y}px`;point.element.style.transform=`translate(-50%,-50%) translate3d(0,0,${point.z}px) scale(${.2+fade*.8})`;point.element.style.opacity=String((fade*fade*(.35+depth*.35)).toFixed(3));point.element.style.zIndex=point.z<0?"1":"20"; });
    }

    updateVisual(item) {
        const position=this.getPosition(item),depth=clamp((position.z+item.depth)/(item.depth*2),0,1),scale=.62+depth*.38;
