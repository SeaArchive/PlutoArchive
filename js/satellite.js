...
class Satellite {
    constructor(planetWrapper) {
        ...
        this.items = this.satellites.map((satellite, index) => {
            ...
            const baseSpeed = Number(planetWrapper.dataset[`${prefix}Speed`] || (second ? .34 : .42));
            return { satellite, angle:Number(planetWrapper.dataset[`${prefix}Angle`] || (second ? 145 : 0)), speed:baseSpeed*1.4*(second?4:-1), radius:Number(planetWrapper.dataset[`${prefix}Radius`] || (second?48:78)), depth:Number(planetWrapper.dataset[`${prefix}Depth`] || (second?34:52)), trailLength:252, trailTimer:0, trailInterval:.1, trail:[] };
        });
